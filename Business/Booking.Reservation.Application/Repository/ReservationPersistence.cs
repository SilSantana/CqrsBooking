...
namespace Booking.Reservation.Application.Repository
{
    internal class ReservationPersistence
    {
        ...
        public ReservationPersistence(IEventPublisher eventPublisher, IRavenDocumentStoreHolder ravenDocumentStore)
        {
        ...
        internal async Task SaveReservationAsync(ReservationDetail reservation)
        {
            using (var session = _ravenDocumentStore.Store.OpenAsyncSession())
            {
                await session.StoreAsync(reservation, reservation.Identifier.ToString());
